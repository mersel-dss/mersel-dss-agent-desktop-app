 ...
 *
 * macOS notlu: `agent` jar'ı içinde imzasız (ad-hoc) bir native taşır
 * (libpkcs11wrapper.jnilib). macOS'ta bu native, build'de `pnpm fetch-services`
 * SONRASI `scripts/sign-bundled-natives.sh` ile Developer ID + timestamp'le
 * yeniden imzalanır; aksi halde notarization patlar. Bu yüzden agent her
 * platformda GÖMÜLÜR; imzalama adımı macOS notarization'ı geçirir.
 *
